...
namespace MattEland.Ani.Alfred.Tests
{
    ...
    public abstract class MockEnabledAlfredTestBase : AlfredTestBase
    {
        ...
        protected Mock<ISearchController> BuildMockSearchController()
        {
            ...
            mock.SetupGet(m => m.StatusMessage)
                .Returns("No searches have been made yet.");

            mock.SetupGet(m => m.Results)
                .Returns(Container.ProvideCollection<ISearchResult>());

            mock.SetupGet(m => m.SearchHistory)
                .Returns(Container.ProvideCollection<SearchHistoryEntry>);
